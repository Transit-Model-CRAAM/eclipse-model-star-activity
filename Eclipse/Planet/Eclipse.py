...
class Eclipse:
    ...
    def criarEclipse(self, anim=True, plot=True):
        ...
        ****parâmetros atribuidos ao planeta****
        ...
        my_func.curvaLuzCME.argtypes = c_double, c_double, c_int, c_double, POINTER(
            c_double), POINTER(c_double), c_double, POINTER(c_double), c_double
        ...
        if (anim):
            ...
            if (self.estrela_.cme):
                self.addCME(rangeloop, xplan, yplan, raioPlanetaPixel,
                            kk2, numAux, ims, ax1, kk, my_func, plota)
                ...
                for lua in self.planeta_.luas:
                    ...
                    self.addLua(rangeloop, xplan, yplan, raioPlanetaPixel, kk2,
                                maxCurvaLuz, numAux, ims, ax1, kk, my_func, plota, lua)
            else:
                if not self.estrela_.useFits:
                    ...
                    em = self.getMatrizTransformada(
                        self.estrela_.estrelaMatriz[0])
                    tam = len(rangeloop)
                    n_imgs = len(self.estrela_.estrelaMatriz)
                    for i in range(0, tam):
                        # índice da imagem atual
                        idx_imagem = int(i * n_imgs / tam)
                        ...
                        if (plota and self.curvaLuz[rangeloop[i]] != 1 and numAux < 200):
                            ...
                            im = ax1.imshow(
                                np.log10(image) * plan, cmap='copper', aspect='equal', animated=True)
                            ###
            ...
            # ax2.axis([-self.tempoTotal/2,self.tempoTotal/2,min(self.curvaLuz)-0.001,1.005])

            ax2.plot(self.tempoHoras, self.curvaLuz, label='Curva de Luz')

            # Definindo os limites dos eixos
            ax2.axis([-self.tempoTotal/2, self.tempoTotal /
                     2, min(self.curvaLuz)-0.001, 1.005])
            ...
            ani = animation.ArtistAnimation(
                fig, ims, interval=50, blit=True, repeat_delay=0.1)
        ...
        # Sem animação de trânsito
            ...
            if (self.estrela_.cme):
                self.addCME_animFalse(
                    rangeloop, xplan, yplan, raioPlanetaPixel, kk2, kk, my_func)
    ...
    def addCME(self, rangeloop, xplan, yplan, raioPlanetaPixel, kk2, numAux, ims, ax1, kk, my_func, plota):
        for i in range(0, len(rangeloop)):
            x0 = xplan[i]
            y0 = yplan[i]

            # Adicionar aqui a cme
            temperatura_cme = self.estrela_.cme.temperatura - \
                self.estrela_.cme.taxa_esfriamento*i
            raio_cme = int(self.estrela_.cme.raio +
                           self.estrela_.cme.velocidade * i)

            # quando a altura da cme atingir o planeta (colocar caluclo da velocidade da cme por tempo de trânsito)

            if (self.estrela_.cme.altura_inicial + self.estrela_.cme.velocidade * i * (self.getTempoTransito() / len(rangeloop)) >= self.planeta_.semiEixoUA):
                # opacidade CME
                opacidade_cme = self.estrela_.cme.opacidade
                # criando a matriz coroa
                coroa = self.estrela_.ejecaoDeMassa(temperatura_cme, raio_cme, opacidade_cme)

                # Soma entre cme e planeta e depois multiplica o resultado pela estrela
                em = self.getMatrizTransformada(coroa)

                # Criando a matriz com a cme
                estrela_manchada_cme = np.copy(self.estrela_matriz)
                estrela_manchada_cme[coroa != 0] = coroa[coroa != 0]

                maxCurvaLuz = np.sum(estrela_manchada_cme)

                self.curvaLuz[rangeloop[i]] = my_func.curvaLuzCME(
                    x0, y0, self.tamanhoMatriz, raioPlanetaPixel, self.ejecaoMassa, kk2, maxCurvaLuz, em, opacidade_cme)
            ...
            if (plota and self.curvaLuz[rangeloop[i]] != 1 and numAux < 200):
                ...
                plan = np.zeros(self.tamanhoMatriz*self.tamanhoMatriz) + \
                    1.  # cria a base do planeta (cheio de 1)
                # cria uma base para a coroa (cheio de 0)
                coroa_array = np.zeros(self.tamanhoMatriz*self.tamanhoMatriz)
                ...
                if (self.estrela_.cme.altura_inicial + self.estrela_.cme.velocidade * i * (self.getTempoTransito() / len(rangeloop)) >= self.planeta_.semiEixoUA):
                    ...
                    # indices que sao sobrepostos entre cme e planeta
                    common_indices = np.intersect1d(ii, jj)
                    coroa_array[common_indices] = coroa[common_indices] * \
                        opacidade_cme
                    coroa_array = coroa_array.reshape(
                        self.tamanhoMatriz, self.tamanhoMatriz)  # transforma em vetor
                    # multiplica o planeta na estrela e soma o valor da cme
                    plot_estrela = estrela_manchada_cme*plan + coroa_array
                    im = ax1.imshow(
                        plot_estrela, cmap=self.cmap, animated=True)
                else:
                    im = ax1.imshow(estrela_manchada_cme*plan,
    ...
    def addCME_animFalse(self, rangeloop, xplan, yplan, raioPlanetaPixel, kk2, kk, my_func):
        for i in range(0, len(rangeloop)):
            x0 = xplan[i]
            y0 = yplan[i]
    ...
    def cme(self, temperatura, raio):
        p0 = (self.estrela_.cme.p0x, self.estrela_.cme.p0y)
        p1 = (self.estrela_.cme.p1x, self.estrela_.cme.p1y)
    ...
    def cmeNoTransito(self, em, temperatura, raio):
        if self.estrela_.cme.geometriaCME == GeometriaCME.VISAO_PERFIL: 
            return self.cmeNoTransitoGota(em, temperatura, raio)
            

        p0 = (self.estrela_.cme.p0x, self.estrela_.cme.p0y)
        p1 = (self.estrela_.cme.p1x, self.estrela_.cme.p1y)
        # intensidade = self.estrela_.cme.opacidade * ((temperatura * 240) / self.estrela_.temperaturaEfetiva) + (1 - self.estrela_.cme.opacidade) * 240
        intensidade = (temperatura * 240) / self.estrela_.temperaturaEfetiva

        cv.line(em, p0, p1, intensidade, raio)
        return em

    def cmeNoTransitoGota(self, coroa, temperatura, raio):
        # latitude
        # longitude
        # inclinacao
        # shape
        ...
        ponta_superior = (self.estrela_.cme.p0x, self.estrela_.cme.p0y)

        # (p1x, p1y) será o centro da base redonda
        centro_base = (self.estrela_.cme.p1x, self.estrela_.cme.p1y)

        # 'raio' define o quão "gorda" será a base da gota
        raio_base = raio

        intensidade = self.estrela_.cme.opacidade * \
            ((temperatura * 240) / self.estrela_.temperaturaEfetiva) + \
            (1 - self.estrela_.cme.opacidade) * 240
        ...
        self.desenharGota(coroa, ponta_superior, centro_base,
                          raio_base, intensidade)
